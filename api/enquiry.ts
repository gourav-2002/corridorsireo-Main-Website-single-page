...
function userEmailHtml(name: string, formType: string): string {
  ...
  const appUrl = process.env.APP_URL || 'https://corridorsireo.com';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { name, phone, email, formType = 'enquiry', config, preferredDate, preferredTime, message, pickup } = req.body;
  ...
  try {
    ...
    console.error('[enquiry] Email error:', msg);
    return res.status(500).json({ success: false, error: `Email error: ${msg}` });
  }
}
